...
def perform_ner(text):
    ...
    {
        "role": "user",
        "content": 'This is example output json format which includes all the entities provided to be extracted\n{"Maximum": "$500,000", "AD&D Maximum Amount": "$500,000", "Acceleration Max %": "75%","Acceleration Max Amount":"$250,000","Elimination Period":"None", "Coverage Termination Age" : "None","Minimum":"$10,000","Max Common Carrier":"$250,000","Spouse (amount)":"$150,000","Child (amount)": "$125,000","Guarantee Issue Amount": "$200,000"}'
    },
    {
        "role": "assistant",
        "content": 'I will remember and provide output in mentioned format'
    },
    {
        "role": "user",
    ...
    entities_json = json.loads(generated_text[8:-4])
    entities_df = pd.DataFrame.from_dict(json.loads(generated_text[8:-4]),orient='index').rename_axis('Entity',axis=1).rename(columns = {0:'Value'})
    entities_df = entities_df.to_html()
    return entities_json, entities_df
